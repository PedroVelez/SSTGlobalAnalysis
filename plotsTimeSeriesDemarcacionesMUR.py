...
# Serie temporal por años    
    ...
    sstHist = sst.sel(time=slice("2003-01-01", str(currentYear-1)+"-12-31"))
    ...
    ax.plot(df.index,df[2003],label='2003')
...
for i in range(0,len(Titulos)):
    ...
    # Daily data
    data = xr.open_dataset(dataDir+'/sstdMUR_mean_'+titulo_short+'.nc')
    sstd = data.analysed_sst
    data = xr.open_dataset(dataDir+'/sstdMUR_anom_mean_'+titulo_short+'.nc')
    sstd_anom = data.analysed_sst

    
    ## Times series mean Sea Surface Temperature
    Title1  = 'Temperatura superficial (MUR) promedio en el '+ titulo
    File1 = imagesDir + '/sstdMUR_mean_'+titulo_short+'.png'
    ...
    Title3  = 'Temperatura superficial (MUR) en el '+ titulo
    File3 = imagesDir + '/sstdMUR_'+titulo_short+'.png'
